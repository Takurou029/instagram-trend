...
export default function WatchlistPage() {
  ...
  const fetchAllData = useCallback(async (list: string[]) => {
  ...
  const addAccount = async () => {
    ...
    try {
      ...
      setAccountsData(prev => [...prev, data]);
      setUsernameInput('');
    } catch (err) {
  ...
  const removeAccount = (username: string) => {
    ...
    setWatchlist(newList);
    localStorage.setItem('insta_watchlist', JSON.stringify(newList));
    setAccountsData(prev => prev.filter(a => a.username !== username));
  };
      ...
      <main className="main-content" style={{ backgroundColor: '#F8FAFC' }}>
        ...
        {watchlist.length < MAX_ACCOUNTS && (
          <div style={{ backgroundColor: 'white', padding: '24px', borderRadius: '20px', border: '1px solid #F1F5F9', marginBottom: '40px', display: 'flex', gap: '16px', alignItems: 'center', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
            <div style={{ position: 'relative', flex: 1, maxWidth: '400px' }}>
              <input 
                type="text" 
                placeholder="ユーザー名を追加 (例: tcb_official)" 
        ...
        <div className="results-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
          {accountsData.map((acc) => {
            ...
            const latestPost = acc.topPosts?.[0];
            const engagementRate = latestPost ? ((latestPost.likes / followers) * 100).toFixed(1) : '0.0';
            const isHighEngagement = parseFloat(engagementRate) >= 3.0;
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px' }}>
                  <Image src={acc.avatar} alt="" width={56} height={56} unoptimized style={{ borderRadius: '50%', border: '2px solid #F1F5F9', padding: '2px' }} />
                  <div>
          ...
          {watchlist.length === 0 && !isLoading && (
            <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '100px 20px', color: '#94A3B8' }}>
              ...
              <p style={{ fontSize: '15px' }}>定点観測したいアカウントを追加してください</p>
            </div>
          )}
        </div>
      </main>
